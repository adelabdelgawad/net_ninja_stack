...
class AsyncSpeedtest:
    ...
    def __init__(self, source_address: Optional[str] = None, debug: bool = False) -> None:
    ...
    async def fetch(self, session: aiohttp.ClientSession, url: str, method: str = "GET", data: Optional[bytes] = None) -> str:
    ...
    async def get_config(self) -> bool:
        async with aiohttp.ClientSession(connector=self._get_connector()) as session:
            try:
                ...
                config = re.search(r'<client ip="(.*?)" lat="(.*?)" lon="(.*?)" isp="(.*?)"', response)
                if config:
                ...
                await logger.info(self.process_id, e)
    ...
    async def get_best_server(self) -> Optional[Dict[str, Any]]:
        ...
        for line in response.splitlines():
            match = re.search(r'<server url="(http://.*?)" lat="(.*?)" lon="(.*?)" name="(.*?)" country="(.*?)" id="(.*?)"', line)
            if match:
                servers.append({
                    "url": match.group(1),
        ...
        async with aiohttp.ClientSession(connector=self._get_connector()) as session:
            for server in servers:
                distance: float = self.calculate_distance(user_lat, user_lon, server["lat"], server["lon"])
                if distance > 500:
                    continue
                ...
                for _ in range(self.LATENCY_TEST_COUNT):
                    try:
                        ...
                        latency: float = (time.time() - start) * 1000  # Convert to milliseconds
            ...
            try:
                async with session.get(url, timeout=self.TIMEOUT) as response:
                    while True:
                        ...
                        logging.debug(f"Downloaded chunk: {len(chunk)} bytes, Total: {total_data} bytes")
            except Exception as e:
                await logger.info(self.process_id, e)
                logging.debug(f"Download chunk failed: {e}")

        async with aiohttp.ClientSession(connector=self._get_connector()) as session:
            tasks = [download_chunk(session, url + "/random4000x4000.jpg") for _ in range(self.TEST_COUNT)]
            await asyncio.gather(*tasks)

        elapsed_time: float = time.time() - start_time
        self.download = total_data / elapsed_time if elapsed_time else 0.0
        logging.debug(f"Total downloaded data: {total_data} bytes in {elapsed_time:.2f} seconds")
    ...
    async def measure_upload_speed(self) -> None:
        ...
        async with aiohttp.ClientSession(connector=self._get_connector()) as session:
            ...
            try:
                while time.time() - start_time < 10:
                    async with session.post(url + "/upload", data=data, timeout=self.TIMEOUT) as response:
                        await response.read()
                    total_data += self.UPLOAD_CHUNK_SIZE
                    logging.debug(f"Uploaded chunk: {self.UPLOAD_CHUNK_SIZE} bytes, Total: {total_data} bytes")
            except Exception as e:
                await logger.info(self.process_id, e)
        ...
        logging.debug(f"Total uploaded data: {total_data} bytes in {elapsed_time:.2f} seconds")
        logging.debug(f"Upload speed: {self.upload:.2f} Bps")

    def _get_connector(self) -> Optional[aiohttp.TCPConnector]:
        return aiohttp.TCPConnector(local_addr=(self.source_address, 0)) if self.source_address else None
