...
from db.crud import create_log, create_quota_result
...
class WEWebScraper:
    ...
    async def __aenter__(self):
        await create_log(
            self.process_id, f"Launching browser for {self.line.name}"
        )
    ...
    async def __aexit__(self, *args, **kwargs):
        ...
        await create_quota_result(self.result)
    ...
    async def login(self) -> bool:
        try:
            ...
            self.driver.find_element(By.ID, "login_loginid_input_01").send_keys(
                self.line.username
            )
            ...
            self.driver.find_element(By.ID, "login_password_input_01").send_keys(
                self.line.password
            )
            ...
            await create_log(
                self.process_id, f"Login failed for {self.line.name}: {e}"
            )
            logger.error(f"Login failed for {self.line.name}: {e}")
            return False

    async def scrape_overview_page(self) -> bool:
        ...
        try:
            ...
            logger.debug(f"Overview page loaded successfully for {
                         self.line.name}.")
            ...
            self.used = float(
                self.driver.find_element(By.CSS_SELECTOR, _used_selector).text.replace(
                    ",", ""
                )
            )
            ...
            self.usage_percentage = await self._calc_usage_percentage(self.used, self.remaining)
            await create_log(
                self.process_id,
            ...
            logger.error(f"Error extracting overview page for {
                         self.line.name}: {e}")
            return False

    async def scrape_renewal_date_page(self) -> bool:
        ...
        try:
            ...
            await asyncio.sleep(2)
            logger.debug(f"Renewal page loaded successfully for {
                         self.line.name}.")
            ...
            logger.error(f"Error extracting renewal page for {
                         self.line.name}: {e}")
            return False
    ...
    async def _set_renewal_date(self, renewal_date_element: str):
        ...
        try:
            ...
            logger.error(f"Error setting renewal date for {
                         self.line.name}: {e}")

    async def _calc_usage_percentage(self, used: float, remaining: float) -> float:
        try:
