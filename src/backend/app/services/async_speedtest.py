        ...
        async with aiohttp.ClientSession(connector=self._get_connector()) as session:
            try:
                ...
                if config:
                    ...
                    logging.info(
                        f"{self.source_address} Public IP: {self.public_ip}, ISP: {self.isp}")
                    return True
                return False
            except Exception as e:
                await create_log(self.process_id, str(e))
                logging.info(
                    f"{self.source_address} Failed to get config: {e}")
                return False
        ...
        async with aiohttp.ClientSession(connector=self._get_connector()) as session:
            for _ in range(self.LATENCY_TEST_COUNT):
                try:
                    ...
                    await create_log(self.process_id, e)
                    logging.info(
                        f"{self.source_address} Latency test failed: {e}")
                    latencies.append(float("inf"))
        self.ping = mean(latencies) if latencies else float("inf")
        logging.info(f"{self.source_address} Ping latency: {self.ping:.2f} ms")
    ...
    async def measure_download_speed(self) -> None:
        ...
        async def download_chunk(session: aiohttp.ClientSession, url: str):
            ...
            try:
                ...
                await create_log(self.process_id, e)
                logging.info(
                    f"{self.source_address} Download chunk failed: {e}")
        ...
        if elapsed_time:
            self.download = total_data / elapsed_time
            self.download = round(self.download * 8 / (1024 * 1024), 2)

        logging.info(f"{self.source_address} Download speed: {
                     self.download} Kbps")
    ...
    async def measure_upload_speed(self) -> None:
        ...
        async with aiohttp.ClientSession(connector=self._get_connector()) as session:
            ...
            try:
                ...
                await create_log(self.process_id, e)
                logging.info(f"{self.source_address} Upload speed test {
                             url} failed: {e}")

        elapsed_time: float = time.time() - start_time
        if elapsed_time:
            self.upload = total_data / elapsed_time
            self.upload = round(self.upload * 8 / (1024 * 1024), 2)
    
        logging.info(f"{self.source_address} Upload speed: {self.upload} Kbps")
