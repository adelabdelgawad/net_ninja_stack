...
async def drop_tables(engine: AsyncEngine):
    ...
    try:
        ...
        f"Error dropping tables: {e}")


def drop_database():
    f"Dropping database '{settings.database.name}'...")
    try:
        engine = create_engine(BASE_SYNC_URL, echo=False, future=True)
        with engine.connect() as conn:
            conn.execute(
                text(f"DROP DATABASE IF EXISTS {settings.database.name}"))
            conn.commit()  # Add commit for DDL operations
        f"Database '{settings.database.name}' deleted.")
    except Exception as e:
        f"Error deleting database: {e}")
